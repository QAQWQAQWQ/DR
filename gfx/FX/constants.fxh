...
// ------------------    Light          -------------------------
// --------------------------------------------------------------
static const float NIGHT_AMBIENT_BOOST = 2.0f; // can just be baked into the below later ye?


static const float3 DayAmbientMapPosX = float3(0.5, 0.5, 0.55);  // right
static const float3 DayAmbientMapNegX = float3(0.45, 0.45, 0.45);  // left
static const float3 DayAmbientMapPosY = float3(0.33, 0.33, 0.26);  // kills everything
static const float3 DayAmbientMapNegY = float3(0.0, 0.0, 0.0);  // from under
static const float3 DayAmbientMapPosZ = float3(0.0502, 0.06023, 0.1023);  // top
...
static const float3 NightAmbientMapNegZ = float3(0.16, 0.12, 0.2);  // bottom

// NOTE: regular unit ambient colors set from defines. this is night
static const float3 NightAmbientPosX = float3(0.6, 0.6, 0.6);  // right
static const float3 NightAmbientNegX = float3(0.1, 0.1, 0.1);  // left
static const float3 NightAmbientPosY = float3(0.3, 0.3, 0.3);  // kills everything
static const float3 NightAmbientNegY = float3(0.0, 0.0, 0.0);  // from under
static const float3 NightAmbientPosZ = float3(2.0, 2.0, 2.0);  // top
static const float3 NightAmbientNegZ = float3(0.2, 0.2, 0.2);  // bottom
...
static const float CITY_LIGHTS_BLOOM_FACTOR 	= 0.2;
...
static const float MUD_NORMAL_CUTOFF 			= 11.982;
static const float MUD_STRENGHTEN 				= 1.0;

static const float 	SNOW_OPACITY_MIN			= 0.85f;
...
static const float3 SNOW_COLOR 					= float3( 0.36, 0.48, 0.59 );
static const float3 SNOW_WATER_COLOR 			= float3( 0.3, 0.3, 1.4 );
static const float 	SNOW_CLIFFS 				= 5.0f;
static const float 	SNOW_SPEC_GLOSS_MULT 		= 0.2f;
static const float 	SNOW_TILING  				= 0.06f;
...
static const float WATER_RIPPLE_EFFECT = 0.0045;
...
// ------------------    HDR          	-------------------------
// --------------------------------------------------------------
static const float3 LUMINANCE_VECTOR  			= float3(0.3135f, 0.7154f, 0.0721f);

// --------------------------------------------------------------
// ------------------    TREES          -------------------------
// --------------------------------------------------------------
static const float 	TREE_SPECULAR = 0.2f;
static const float 	TREE_ROUGHNESS = 0.3f;
...
static const float  WATER_TIME_SCALE			= 1.5f / 50.0f;
...
static const float3 FOG_COLOR 					= float3( 0.22, 0.38, 0.6 );
...
static const float GB_CAM_MIN = 132.0f;
static const float GB_CAM_MAX = 315.0f;
...
static const float GB_OUTLINE_CUTOFF_SEA = 0.960f; // Magic number to balance cutoff on edges without neighbor (over Sea)
static const float GB_OPACITY_NEAR = 0.95f; // Transparency when camera is near
static const float GB_OPACITY_FAR = 1.0f;  // Transparency when camera is far
static const float BORDER_NIGHT_DESATURATION_MAX = 0.35f; // how much border colors can get desaturated at night. 1.0f is full grey
static const float BORDER_FOW_REMOVAL_FACTOR = 0.9f; // How much of the FOW that is removed from the borders. 1.0f is no FOW
static const float BORDER_LIGHT_REMOVAL_FACTOR = 0.22f; // How much of the light calculations that are removed from the borders. 1.0f is no light
...
// -------------    RIM LIGHT (PDXMESH)   -----------------------
// --------------------------------------------------------------
static const float 	RIM_START 		= 0.45f;
static const float 	RIM_END 		= 0.4f;
